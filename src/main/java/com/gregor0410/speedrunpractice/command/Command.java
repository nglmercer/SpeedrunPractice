    ...
    public static void registerCommands(){
        CommandRegistrationCallback.EVENT.register(((dispatcher, dedicated, environment) -> {
            ...
            dispatcher.register(
                literal("practice")
                    ...
                    .then(literal("world").executes(ctx->{
                        ...
                        player.sendMessage(Text.literal(player.getServerWorld().getRegistryKey().getValue().toString()),false);
                        return 1;
                    }))
            ...
            dispatcher.register(literal("instaperch").executes((ctx)->{
                ctx.getSource().getServer().getCommandManager().executeWithPrefix(ctx.getSource().withSilent().withLevel(4),"/data merge entity @e[type=ender_dragon,limit=1] {DragonPhase:2}");
                return 1;
            }));
        }));
    }
