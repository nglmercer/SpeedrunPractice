 ...
 */
final class LiveInventories implements InventoryAdapter {
    private final LiveAdapter116 adapter;

    LiveInventories(LiveAdapter116 adapter) {
    ...
    public void applyLoadout(PracticePlayer player, Loadout loadout) throws PracticeException {
        ...
        for (Loadout.Item item : loadout.items()) {
            ...
            Item live;
            try {
                live = Registry.ITEM.get(new Identifier(id));
            } catch (RuntimeException bad) {
                SpeedrunLogger.warn("Skipping \"" + item.itemId() + "\": bad item id");
                continue;
            }
            if (live == null || live == Items.AIR) {
                SpeedrunLogger.warn("Skipping \"" + item.itemId() + "\": no such item on 1.16.1");
                continue;
            }
    ...
    public Loadout captureLoadout(PracticePlayer player, String id) throws PracticeException {
        ...
        for (int slot = 0; slot < entity.inventory.main.size(); slot++) {
            ItemStack stack = entity.inventory.main.get(slot);
            if (!stack.isEmpty()) {
                items.add(new Loadout.Item(Registry.ITEM.getId(stack.getItem()).toString(), slot, stack.getCount()));
            }
        }
        for (int slot = 0; slot < entity.inventory.armor.size(); slot++) {
            ItemStack stack = entity.inventory.armor.get(slot);
            if (!stack.isEmpty()) {
                items.add(new Loadout.Item(Registry.ITEM.getId(stack.getItem()).toString(),
                        100 + slot, stack.getCount()));
            }
        }
        for (int slot = 0; slot < entity.inventory.offHand.size(); slot++) {
            ItemStack stack = entity.inventory.offHand.get(slot);
            if (!stack.isEmpty()) {
                items.add(new Loadout.Item(Registry.ITEM.getId(stack.getItem()).toString(), -106, stack.getCount()));
            }
        }
    ...
    private static boolean place(ServerPlayerEntity entity, int slot, ItemStack stack) {
        ...
        switch (area.area()) {
            case MAIN:
                if (slot < 0 || slot >= entity.inventory.main.size()) {
                    return false;
                }
                entity.inventory.main.set(slot, stack);
                return true;
            case ARMOR:
                if (area.index() < 0 || area.index() >= entity.inventory.armor.size()) {
                    return false;
                }
                entity.inventory.armor.set(area.index(), stack);
                return true;
            case OFFHAND:
