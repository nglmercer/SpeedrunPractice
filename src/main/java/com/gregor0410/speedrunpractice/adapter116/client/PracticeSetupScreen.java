 ...
 */
final class PracticeSetupScreen extends PracticeScreenBase {
    ...
    PracticeSetupScreen(PracticeMenuModel.ScenarioScreen model, String customId) {
    ...
    private PracticeSetupScreen(PracticeMenuModel.ScenarioScreen model, String customId,
    ...
    @Override
    protected void init() {
        seedField = new TextFieldWidget(textRenderer, width / 2 - 100, 118, 200, 20,
                new LiteralText("Seed value"));
        seedField.setMaxLength(24);
        seedField.setText(pendingSeedValue);
        children.add(seedField);
        ...
        smallButton(width / 2 + 5, 190, 150, "Back", button -> ClientScreens.openMain());
    }
    ...
    private void reopen(int nextSeed, int nextLoadout) {
        String value = seedField == null ? pendingSeedValue : seedField.getText();
        ClientScreens.open(new PracticeSetupScreen(model, customId, nextSeed, nextLoadout, value));
    }

    private void start() {
        final String source = seedSource();
        final String value = seedField == null ? "" : seedField.getText().trim();
        ...
        ClientScreens.onServer((server, entity, runtime) -> {
            ...
            if ("fixed".equals(source)) {
                if (value.isEmpty()) {
                    entity.sendMessage(new LiteralText("Enter a seed value or pick another source."), false);
                    return;
                }
                try {
                    Long.parseLong(value);
                } catch (NumberFormatException bad) {
                    entity.sendMessage(new LiteralText("\"" + value + "\" is not a number."), false);
                    return;
                }
            ...
            if (customId != null) {
                session = runtime.startCustom(customId, settings, ClientScreens.handle(entity));
            } else {
                session = runtime.startPractice(preset.type(), settings, ClientScreens.handle(entity));
            }
            String name = preset.type() == PracticeType.CUSTOM ? customId : preset.displayName();
            entity.sendMessage(new LiteralText("Started " + name + " on seed " + session.seed() + "."), false);
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        int y = 34;
        line(matrices, "Seed source, loadout, then Start.", y, 0xA0A0A0);
        y += 12;
        List<String> notes = model.unsupportedNotes();
        for (int i = 0; i < notes.size() && i < 2; i++) {
            line(matrices, notes.get(i), y, 0xFF5555);
            y += 10;
        }
        line(matrices, "Seed value (fixed source):", 106, 0xA0A0A0);
        seedField.render(matrices, mouseX, mouseY, delta);
        y = 144;
        ...
        for (Map.Entry<String, String> option : options.entrySet()) {
            ...
            line(matrices, option.getKey() + " = " + option.getValue(), y, 0x808080);
            y += 10;
            shown++;
        }
        if (options.size() > shown) {
            line(matrices, "+" + (options.size() - shown) + " more defaults (edit via config)", y, 0x808080);
        }
    }
