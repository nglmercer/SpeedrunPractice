...
final class PracticeMainScreen121 extends PracticeScreenBase121 {
    PracticeMainScreen121() {
    ...
    protected void init() {
        ...
        smallButton(leftX, baseY, 150, "Loadouts", button -> ClientScreens121.openLoadouts());
        smallButton(rightX, baseY, 150, "Statistics", button -> ClientScreens121.openStats());
        smallButton(leftX, baseY + 24, 150, "Seed Search", button -> ClientScreens121.openSeeds());
        smallButton(rightX, baseY + 24, 150, "Close", button -> ClientScreens121.open(null));
    }
    ...
    private void openSetup(PracticeMenuModel.MenuEntry entry) {
        ClientScreens121.onServer((server, entity, runtime) -> {
            if (entry.type() == PracticeType.CUSTOM) {
                List<String> customs = ClientData121.customIds(runtime);
                if (customs.isEmpty()) {
