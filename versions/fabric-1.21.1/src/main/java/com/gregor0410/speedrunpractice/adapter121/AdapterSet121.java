...
/**
 * 1.21.1 adapter set (modern compatibility build). Skeleton: live-Minecraft
 * methods fail with a domain exception until ported against 1.21.1 mappings
 * (plan section 26). Shared code is never modified for mapping names.
 */
public final class AdapterSet121 implements MinecraftAdapter {
    ...
    private static PracticeException.AdapterException pending(String operation) {
        return new PracticeException.AdapterException("1.21.1 adapter: " + operation + " is pending the port",
                PENDING + " (" + operation + ")");
    }
