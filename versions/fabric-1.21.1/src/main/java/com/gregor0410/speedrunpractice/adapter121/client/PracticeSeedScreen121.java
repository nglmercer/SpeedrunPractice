 ...
 * for real.
 */
final class PracticeSeedScreen121 extends PracticeScreenBase121 {
    ...
    PracticeSeedScreen121(Data data) {
        super("Seed Search");
        this.data = data;
    }
    ...
    protected void init() {
        ...
        smallButton(width / 2 + 5, 150, 150, "Start search", button ->
                ClientScreens121.onServer((server, entity, runtime) -> {
                    ...
                    entity.sendMessage(Text.literal(started), false);
        ...
        smallButton(width / 2 + 5, 174, 150, "Back", button -> ClientScreens121.openMain());
    }
    ...
    private static void refreshOnClient(int presetIdx, PracticeRuntime runtime) {
        ...
        MinecraftClient.getInstance()
                .execute(() -> ClientScreens121.open(new PracticeSeedScreen121(fresh)));
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int y = 34;
        if (data.presets.isEmpty()) {
            line(context, "No presets yet. Save searches to run them here.", y, 0xA0A0A0);
            y += 12;
        }
        if (data.progress == null) {
            line(context, "No seed search has been started.", y, 0xA0A0A0);
        } else {
            line(context, "Tested: " + data.progress.seedsTested()
            ...
            line(context, "State: " + state + "  Failed: " + data.progress.seedsFailed(), y, 0xA0A0A0);
            y += 12;
            for (int i = 0; i < data.seeds.size(); i++) {
                line(context, "Seed: " + data.seeds.get(i), y, 0x55FF55);
                y += 10;
            }
        }
    }
