 ...
 */
final class PracticeSetupScreen121 extends PracticeScreenBase121 {
    ...
    PracticeSetupScreen121(PracticeMenuModel.ScenarioScreen model, String customId) {
    ...
    private PracticeSetupScreen121(PracticeMenuModel.ScenarioScreen model, String customId,
    ...
    @Override
    protected void init() {
        seedField = new TextFieldWidget(client.textRenderer, width / 2 - 100, 118, 200, 20,
                Text.literal("Seed value"));
        ...
        smallButton(width / 2 + 5, 190, 150, "Back", button -> ClientScreens121.openMain());
    }
    ...
    private void reopen(int nextSeed, int nextLoadout) {
        ...
        ClientScreens121.open(new PracticeSetupScreen121(model, customId, nextSeed, nextLoadout, value));
    }
    ...
    private void start() {
        ...
        ClientScreens121.onServer((server, entity, runtime) -> {
            ...
            if ("fixed".equals(source)) {
                if (value.isEmpty()) {
                    entity.sendMessage(Text.literal(
                            "Enter a seed value or pick another source."), false);
                    return;
                }
                try {
                    Long.parseLong(value);
                } catch (NumberFormatException bad) {
                    entity.sendMessage(Text.literal("\"" + value + "\" is not a number."), false);
                    return;
                }
            ...
            if (customId != null) {
                session = runtime.startCustom(customId, settings, ClientScreens121.handle(entity));
            } else {
                session = runtime.startPractice(preset.type(), settings, ClientScreens121.handle(entity));
            }
            String name = preset.type() == PracticeType.CUSTOM ? customId : preset.displayName();
            entity.sendMessage(Text.literal(
                    "Started " + name + " on seed " + session.seed() + "."), false);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        int y = 34;
        line(context, "Seed source, loadout, then Start.", y, 0xA0A0A0);
        y += 12;
        List<String> notes = model.unsupportedNotes();
        for (int i = 0; i < notes.size() && i < 2; i++) {
            line(context, notes.get(i), y, 0xFF5555);
            y += 10;
        }
        line(context, "Seed value (fixed source):", 106, 0xA0A0A0);
        y = 144;
        ...
        for (Map.Entry<String, String> option : options.entrySet()) {
            ...
            line(context, option.getKey() + " = " + option.getValue(), y, 0x808080);
            y += 10;
            shown++;
        }
        if (options.size() > shown) {
            line(context, "+" + (options.size() - shown) + " more defaults (edit via config)", y, 0x808080);
        }
    }
