...
import com.gregor0410.speedrunpractice.adapter116.client.ClientScreens;
...
/**
 * GUI entry points on the 1.16.1 client. Calls from any thread are
 * marshalled to the client thread; on a dedicated server every call fails
 * readably and {@link #isAvailable()} is false. Only {@link ClientScreens}
 * touches client classes, and only behind the environment check, so this
 * class stays load-safe on servers.
 */
final class LiveGui implements GuiAdapter {
    @Override
    ...
    private void openClient(Runnable opener) throws PracticeException {
        ...
        ClientScreens.openOnClientThread(opener);
    }
    ...
    private static final class OpenScenario implements Runnable {
        ...
        @Override
        public void run() {
            ClientScreens.openSetup(preset, customId);
        }
    }
