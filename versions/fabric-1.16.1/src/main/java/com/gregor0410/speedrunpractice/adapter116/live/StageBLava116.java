...
final class StageBLava116 {
    ...
    // TEMP round-7 spring-gate read trace: revert after use.
    private static boolean traceSpring;
    private static int traceChunkX;
    private static int traceChunkZ;
    ...
    static boolean verify(MinecraftServer server, ServerWorld overworld, long seed,
        ...
        SpeedrunLogger.warn("PROBELAVA116-DIAG seed=" + seed + " village="
                + (village == null ? "none" : village.x + "," + village.z));
        ...
        try {
                ...
                for (int chunkZ = minChunkZ; chunkZ <= maxChunkZ; chunkZ++) {
                    ...
                    for (GenerationStep.Feature step : steps) {
                        ...
                        for (int index = 0; index < features.size(); index++) {
                            ...
                            // TEMP round-7/8 gate trace: revert after use.
                            boolean trace = seed == 17L
                                    && (kind == LavaKind.SPRING || kind == LavaKind.LAKE);
                            if (trace) {
                                traceSpring = true;
                                traceChunkX = chunkX;
                                traceChunkZ = chunkZ;
                            ...
                            if (kind == LavaKind.LAKE || kind == LavaKind.SPRING) {
                                level.noteTarget(kind, chunkX, chunkZ, index);
                            }
                            try {
                                boolean placed = configured.generate(level, accessor, generator,
                                        random, origin);
                                // TEMP round-3 basin trace: revert after use.
                                if (seed == 17L && kind == LavaKind.LAKE) {
                                    SpeedrunLogger.warn("PROBELAVA116-TRY seed=17 chunk="
                                            + chunkX + "," + chunkZ + " placed=" + placed
                                            + " index=" + index + " fluid=" + lakeFluid(configured));
                                }
                                if (trace) {
                                    SpeedrunLogger.warn("PROBELAVA116-STRY seed=17 chunk="
                                            + chunkX + "," + chunkZ + " index=" + index + " cfg="
                                            + springCfg(configured) + " placed=" + placed);
                                }
                            } catch (RuntimeException unsupported) {
                                // TEMP round-6: log swallowed feature failures.
                                if (seed == 17L && kind == LavaKind.LAKE) {
                                    SpeedrunLogger.warn("PROBELAVA116-TRY seed=17 chunk="
                                            + chunkX + "," + chunkZ + " placed=ERROR err="
                                            + unsupported.getClass().getSimpleName());
                                }
                                if (trace) {
                                    SpeedrunLogger.warn("PROBELAVA116-STRY seed=17 chunk="
                                            + chunkX + "," + chunkZ + " index=" + index
                                            + " placed=ERROR err="
                                            + unsupported.getClass().getSimpleName());
                                }
            ...
            if (level.foundLava()) {
                ...
                SpeedrunLogger.warn("PROBELAVA116-DIAG seed=" + seed + " lavaAt=" + first
                        + " noiseSurf=" + noiseSurface + " how=" + level.firstLavaHow()
                        + " failed=" + failed);
            }
    ...
    // TEMP round-6 diagnostics: revert after use.
    private static String lakeFluid(ConfiguredFeature<?, ?> configured) {
        try {
            FeatureConfig config = configured.config;
            if (config instanceof DecoratedFeatureConfig) {
                ConfiguredFeature<?, ?> inner = ((DecoratedFeatureConfig) config).feature;
                config = inner == null ? null : inner.config;
            }
            if (config instanceof SingleStateFeatureConfig) {
                BlockState state = ((SingleStateFeatureConfig) config).state;
                if (state != null) {
    ...
    // TEMP round-7 diagnostics: revert after use.
    private static String springCfg(ConfiguredFeature<?, ?> configured) {
        try {
            FeatureConfig config = configured.config;
            if (config instanceof DecoratedFeatureConfig) {
                ConfiguredFeature<?, ?> inner = ((DecoratedFeatureConfig) config).feature;
            ...
            if (config instanceof SpringFeatureConfig) {
                SpringFeatureConfig spring = (SpringFeatureConfig) config;
                StringBuilder valid = new StringBuilder();
                for (Block block : spring.validBlocks) {
                    valid.append(Registry.BLOCK.getId(block)).append(',');
                }
                return "reqBelow=" + spring.requiresBlockBelow + " rock=" + spring.rockCount
                        + " hole=" + spring.holeCount + " valid=[" + valid + "]";
            }
        } catch (RuntimeException ignored) {
        }
        return "?";
    ...
    private static final class ColumnLevel implements ServerWorldAccess {
        ...
        private int built;
        // TEMP round-4 budget trip-wire: revert after use.
        ...
        /** Placing kind/chunk/index of the feature currently generating. */
        ...
        void noteTarget(LavaKind kind, int chunkX, int chunkZ, int index) {
            currentHow = kind + "@" + chunkX + "," + chunkZ + "i" + index;
        }

        // TEMP probe diagnostics: revert after verification.
        int noiseSurfaceAt(int x, int z) {
            try {
                return getTopY(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, x, z);
        ...
        public BlockState getBlockState(BlockPos pos) {
            ...
            if (override != null) {
                return springRead(pos, override, "override");
            }
            if (pos.getY() < 0 || pos.getY() >= WORLD_HEIGHT) {
                return springRead(pos, Blocks.AIR.getDefaultState(), "void");
            }
            ProtoChunk chunk = surfaced(Math.floorDiv(pos.getX(), 16), Math.floorDiv(pos.getZ(), 16));
            if (chunk == null) {
                return springRead(pos, Blocks.AIR.getDefaultState(), "unserved");
            }
            try {
                BlockState state = chunk.getBlockState(pos);
                return springRead(pos,
                        state == null ? Blocks.AIR.getDefaultState() : state, "scratch");
            } catch (RuntimeException outOfRange) {
                return springRead(pos, Blocks.AIR.getDefaultState(), "range");
            }
        }

        // TEMP round-7/8 gate read trace: revert after use.
        private BlockState springRead(BlockPos pos, BlockState state, String src) {
            if (traceSpring && seed == 17L && pos.getX() >= 20 && pos.getX() <= 56
                    && pos.getZ() >= -40 && pos.getZ() <= -8 && pos.getY() >= 68
                    && pos.getY() <= 80) {
                SpeedrunLogger.warn("PROBELAVA116-SREAD seed=17 chunk=" + traceChunkX + ","
                        + traceChunkZ + " index=" + traceIndex + " at=" + pos.getX() + ","
                        + pos.getY() + "," + pos.getZ() + " block="
                        + Registry.BLOCK.getId(state.getBlock()) + " src=" + src);
            }
            return state;
        }
        ...
        public boolean setBlockState(BlockPos pos, BlockState state, int flags, int maxUpdateDepth) {
            ...
            // TEMP round-3 basin trace: revert after use. Logs every replay
            // write near seed 17's lost lake (36,73,-21) to identify the
            // cross-chunk writer, if any.
            if (seed == 17L && pos.getX() >= 16 && pos.getX() <= 64
                    && pos.getZ() >= -48 && pos.getZ() <= 8 && pos.getY() >= 40
                    && pos.getY() <= 100) {
                SpeedrunLogger.warn("PROBELAVA116-TRACE seed=17 at=" + pos.getX() + ","
                        + pos.getY() + "," + pos.getZ() + " block="
        ...
        private ProtoChunk surfaced(int chunkX, int chunkZ) {
            ...
            try {
                ...
                // TEMP round-6 diagnostics: revert after use.
                if (seed == 17L) {
                    SpeedrunLogger.warn("PROBELAVA116-SURF seed=17 chunk=" + chunkX + ","
                            + chunkZ + " ok=false err=" + badChunk.getClass().getSimpleName());
                }
                return null;
            }
            if (fresh == null) {
