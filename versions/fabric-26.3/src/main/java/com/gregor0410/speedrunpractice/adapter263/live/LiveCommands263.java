...
/**
 * Builds real Brigadier nodes from the shared command tree on 26.3. Unlike
 * 1.16.1 there is no legacy tree here, so every shared node registers,
 * including the full {@code seed} subtree.
 */
final class LiveCommands263 implements CommandAdapter {
    @Override
    public void register(final PracticeCommands.Node root, final CommandExecutor executor) {
        ...
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            LiteralArgumentBuilder<CommandSourceStack> builder = literal(root.name());
            ...
            SpeedrunLogger.info("Registered /" + root.name() + " (" + root.children().size()
                    + " shared children) on the 26.3 dispatcher");
        });
    }

    private static ArgumentBuilder<CommandSourceStack, ?> build(PracticeCommands.Node node,
        ...
        if (node.name().startsWith("<") && !node.args().isEmpty()) {
            ...
            LiteralArgumentBuilder<CommandSourceStack> literal = literal(node.name());
            if (node.action() != null) {
    ...
    private static int run(CommandContext<CommandSourceStack> context, String action,
        ...
        try {
            ...
            context.getSource().sendFailure(Component.literal(failure.getUserMessage()));
            return 0;
    ...
    private static final class View implements CommandContextView {
        ...
        private View(CommandContext<CommandSourceStack> context, String action) {
        ...
        public PracticePlayer player() throws PracticeException {
            ...
            return new LivePlayer263(player);
        }
